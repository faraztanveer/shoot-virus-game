...
var loadImages = function() {
    ...
    var onImgLoad = function() {
        ...
        if (imgComplete == imgNumber) {
            ...
            // for (var i = 0; i < 10; ++i) {
            //   buildings.push(
            //     Math.floor(Math.random() * (height - 300 - delta) + 10) - 100
            //   );

            //   buildingsDir.push(Math.random() > 0.5);
            // }
            // drawCanvas();
        }
    };
    ...
    virus.onload = onImgLoad;
    target = new Image();
    target.src = "assets/Target.png";
    target.onload = onImgLoad;

    score_board = new Image();
    score_board.src = "assets/score-board.png";
    score_board.onload = onImgLoad;


    title = new Image();
    title.src = "assets/title.png";
    title.onload = onImgLoad;
...
function enginestep() {
    ...
    if (!gameover) {
        ...
        for (var i = 0; i < enemies.length; i++) {
            ...
            if (!en.des) {
                ...
                ctx.drawImage(virus, en.x - 90, en.y - 90, virus.width / 2, virus.height / 2);
        ...
        ctx.drawImage(score_board, 10, 20, score_board.width / 1.4, score_board.height / 1.4);
        ctx.fillText(score, 200, 50);
        ctx.drawImage(title, 380, 40, title.width / 1.3, title.height / 1.3);
        ctx.font = "100px arial";

        ctx.fillText(seconds, 1100, 50);
    ...
    ctx.drawImage(target, pointer.x - (target.width / 4), pointer.y - (target.height / 4), target.width / 2, target.height / 2);
    // target
    // ctx.beginPath();
